...
(defmacro %non-adaptive-eq-hash (address)
...
;;; This hash function was historically used both in EQ hash tables
;;; and as parts of EQL-HASH, EQUAL-HASH and EQUALP-HASH. EQ hash
;;; tables now adapt their hash function to the actual keys.
(define-eq-hash (non-adaptive-eq-hash non-adaptive-eq-hash*) (address)
  (%non-adaptive-eq-hash address))

(defconstant +hft-eq-mid+ 0)

;;; This is a family of hash functions (one for +HFT-EQ-MID+ and one
;;; for each bit in a WORD).
(define-eq-hash (eq-hash/common eq-hash/common*) (address state)
  ...
  ;; DEFINE-REMHASH), but the increased code is a drag on e.g.
  ;; compiler performance. Crucially, the IF is lifted out of the
  ;; REHASHing loop by WITH-EQ-HASH*-INLINED.
  (if (not (eq state +hft-eq-mid+))
      ;; EQ-HASH/SMALL
      ;;
      ;; GUESS-EQ-HASH-FUN may switch to this function.
      (+ (ash address (- (truly-the (mod #.sb-vm:n-word-bits) state)))
         ;; 15 is about the number of bits on a gencgc page. The exact
         ;; value is not that important.
         (ash address -15))
      ;; EQ-HASH/MID
      ;;
      ;; This was the default and only EQ hash function for a long
      ;; time, and we still use it as a safer but often slower option
      ;; than the really aggressive EQ-HASH/SMALL just above.
      ;;
      ;; ADAPT-EQ-HASH-FUN-BY-N-COLLISIONS switches to this hash
      ;; function if there are too many collisions for EQ-HASH/SMALL,
      ;; but not before +EQ-MIN-N-BUCKETS-FOR-COLLISION-COUNTING+.
      (%non-adaptive-eq-hash address)))

(defmacro eq-hash/small-state (shift)
  `(the (integer 1 (#.sb-vm:n-word-bits)) ,shift))

;;; Finally, ADAPT-EQ-HASH-FUN-BY-N-COLLISIONS may switch from
;;; +HFT-EQ-MID+ from to +HFT-GENERAL+, which should be a safe
;;; fallback that handles most key distributions gracefully. This
;;; should be required very rarely, so this is not packed into
;;; EQ-HASH/COMMON.
(define-eq-hash (eq-hash/general eq-hash/general*) (address)
...
;;; the state is not +HFT-FLAT+.
(defmacro with-eq-hash*-inlined ((state &optional count-collisions-p)
                                 &body body)
  `(cond
     ;; EQ-HASH/SMALL (the first branch of EQ-HASH/COMMON)
     ((plusp ,state)
      (macrolet ((eq-hash* (key)
        ...
        (let (,@(when count-collisions-p
          ...
          ,@body)))
     ;; The second branch of EQ-HASH/COMMON.
  ...
  (let* ((kv-vector (hash-table-pairs table))
    ...
    (let ((unchanged-bits (ldb (byte #.sb-vm:n-word-bits 0)
      ...
      (set-hash-fun
       ...
       (if (/= (ldb (byte #.sb-vm:n-fixnum-tag-bits 0) unchanged-bits)
               #.(1- (ash 1 sb-vm:n-fixnum-tag-bits)))
           ;; Mix of fixnums and non-fixnums
           +hft-eq-mid+
           ;; All fixnums or no fixnums
           (let* ((min-right-shift
  ...
  `(defun ,name (key table default)
     ...
     (let* ((hash-table (truly-the hash-table table))
       ...
       (if ,(cond ((eq test 'eq) nil)
                  ...
                  (t t))
           (loop for i upfrom 2 upto (* hwm 2) by 2
                 when (,test key (aref kv-vector i))
                   return (values (aref kv-vector (1+ i)) t)
                 finally (return (values default nil)))
           ;; Unroll the loop a tiny bit.
           (let ((hwm/2 (ash hwm -1)))
             (loop for i upfrom 2 upto (* hwm/2 4) by 4
                   do (when (eq key (aref kv-vector i))
                        (return-from ,name
                          (values (aref kv-vector (+ i 1)) t)))
                      (when (eq key (aref kv-vector (+ i 2)))
                        (return-from ,name
                          (values (aref kv-vector (+ i 3)) t))))
             (unless (zerop (logand hwm 1))
               (when (eq key (aref kv-vector (* hwm 2)))
                 (return-from ,name
                   (values (aref kv-vector (+ (* hwm 2) 1)) t))))
             (values default nil))))))
                 ...
                 (loop for i upfrom 2 upto (* hwm/2 4) by 4
                       do (let ((ht-key (aref kv-vector i)))
                            (when (eq key ht-key)
                              (setf (aref kv-vector (1+ i)) value)
                              (return-from ,name value)))
                          (let ((ht-key (aref kv-vector (+ i 2))))
                            (when (eq key ht-key)
                              (setf (aref kv-vector (+ i 3)) value)
                              (return-from ,name value))))
                 (unless (zerop (logand hwm 1))
                   (let ((i (* 2 hwm)))
...
(defun compute-flat-hash-address-hashing-p (table)
  ...
  (let* ((hash-table (truly-the hash-table table))
    ...
    (if (eq (hash-table-test table) 'eq)
        (loop
          for i upfrom 2 upto (* hwm 2) by 2
                ...
                (let* ((stored-hash (aref hash-vector i))
                       (bucket
                         (cond ((/= stored-hash +magic-hash-vector-value+)
                               ...
                               (t
                                (mask-hash (non-adaptive-eq-hash* pair-key)
                                           mask)))))
        ...
        (let ((old-addr-hashing-p
          ...
          (let ((next-free
                  (cond ((eq new-index-vector (hash-table-index-vector table))
                         ;; The index-vector hasn't grown, so we only
                         ;; need to extend the next-vector.
          ...
          (ecase test
            (eq
             (case hash-fun-state
               ...
               ((#.+hft-general+)
                (pick-table-methods-1
                 synchronized gethash/eq-hash/general puthash/eq-hash/general
                 remhash/eq-hash/general hash-fun-state))
               (t
                (pick-table-methods-1
            ...
            (eql
             (if (eql hash-fun-state +hft-flat+)
                 ...
                 (pick-table-methods-1
                  ...
                  remhash/eql-hash hash-fun-state)))
...
(flet ((insert-at (index hash-table key hash address-based-p value)
  ...
  (define-ht-setter puthash/eq-hash/general eq eq-hash/general*))
